...
def show_pcl(pcl):
    ...
    vis.create_window(window_name = 'S2_Ex1', visible=True)
    ...
    right_key = 262
    vis.register_key_callback(right_key, vis.close())
    vis.poll_events()
    vis.update_renderer()
    vis.run()
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    intensity_map[np.int_(lidar_pcl_int[:,0]), np.int_(lidar_pcl_int[:,1])] = np.amax(lidar_pcl_int[:,3])/2 * lidar_pcl_int[:,3] * 255/(np.amax(lidar_pcl_int[:,3])-np.amin(lidar_pcl_int[:,3]))
    ...
    height_map[np.int_(lidar_pcl_int[:,0]), np.int_(lidar_pcl_int[:,1])] = (lidar_pcl_int[:,2]-configs.lim_z[0])/(configs.lim_z[1]-configs.lim_z[0])
    ...
    # heatmapshow = None
    # heatmapshow = cv2.normalize(height_intensity, heatmapshow, alpha=0, beta = 255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U)
    # heatmapshow = cv2.applyColorMap(heatmapshow, cv2.COLORMAP_JET)
    # cv2.imshow('Heatmap', heatmapshow)
    # cv2.imwrite('./img/Ex5/heatmap.png', heatmapshow)
